  ...
  return <div>
    ...
    <div className="absolute px-8 py-2 bg-gradient-to-b from-black w-full">
      ...
      <form className="absolute ml-56 mt-40 mx-auto items-center justify-center gap-5 p-20 bg-black right-0 left-0 flex flex-col w-fit">
        ...
        {!isSignForm && (
          <input type="text" placeholder="Full Name" className="px-4 py-2 m-3 rounded-md bg-gray-300" />
        )}
        <input type="text" placeholder="Email address" className="px-4 py-2 m-3 rounded-md bg-gray-300" />
           <input type="password" placeholder="Password" className="px-4 py-2 rounded-md m-3 bg-slate-300" />
          <button onClick={toggleSignInForm} className="mx-auto w-full px-4 items-center justify-center text-center py-2 rounded-md  bg-red-500 gap-5">{isSignForm ? "Sign In" : "Sign Up"}</button>
